...
// Global event system for opening the modal from anywhere
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '36px 36px 28px' }}>
          ...
          <form onSubmit={e => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {isSignup && <Field label="Name" type="text" placeholder="Alex Morgan" autoComplete="name" />}
            <Field label="Email" type="email" placeholder="you@example.com" autoComplete="email" />
            <Field label="Password" type="password"
              placeholder={isSignup ? 'At least 8 characters' : 'Your password'}
              autoComplete={isSignup ? 'new-password' : 'current-password'}
              extra={!isSignup && (
                <a href="#" onClick={e => e.preventDefault()} style={{ fontSize: 12, color: 'var(--terracotta-500)', textDecoration: 'none', fontWeight: 500 }}>
                  Forgot?
                </a>
              )}
            />
...
function Field({ label, type, placeholder, autoComplete, extra }: {
  label: string; type: string; placeholder: string; autoComplete: string; extra?: React.ReactNode;
}) {
...
function SubmitButton({ label }: { label: string }) {
  ...
  return (
    <button type="submit" style={{
      ...
      background: hover ? 'var(--terracotta-500)' : 'var(--terracotta-400)',

...
function PricingPage() {
  return (
    <div data-screen-label="GutHub Pricing">
      <Header />
      <PricingHero />
      <PricingCard />
      <WhatsIncluded />
    ...
    <section style={{
      ...
      <Reveal style={{ maxWidth: 760, margin: '0 auto' }}>
        <div style={{
          ...
          color: 'var(--terracotta-600, #B85A3E)',
    ...
    <section style={{
      ...
      <Reveal y={24} duration={800} style={{
        ...
        <div style={{
          ...
          <i data-lucide="sparkles" style={{ width: 14, height: 14, color: 'var(--terracotta-300)' }}></i>
        ...
        <div style={{ padding: '40px 40px 32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 4, flexWrap: 'wrap' }}>
            ...
            <span style={{
              ...
              color: 'var(--ink-900)', letterSpacing: '-0.025em', lineHeight: 1.1,
            }}>$9.95</span>
          ...
          <div style={{
            ...
            Founding member · locked in for life
          </div>
          ...
          <div style={{
            marginTop: 14, fontSize: 13, color: 'var(--ink-500)', textAlign: 'center',
            display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, flexWrap: 'wrap',
          }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              <i data-lucide="shield-check" style={{ width: 14, height: 14 }}></i> Secure checkout
            </span>
            <span>·</span>
...
function WhatsIncluded() {
  const features = [
    { icon: 'message-circle', title: 'Unlimited chat', body: 'Ask anything, anytime. Your AI guide remembers your history and adapts to you.' },
    { icon: 'camera', title: 'Snap & know', body: 'Photo-to-macros in seconds. Calories, protein, carbs, fat, plus gut-health flags.' },
    { icon: 'calendar-days', title: 'AI meal planner', body: 'Weekly meals built around your diet and preferences. Grocery list included.' },
    { icon: 'target', title: 'Goal tracker', body: 'Set a goal weight. Daily macro targets. Visual progress and gentle nudges.' },
    { icon: 'trending-up', title: 'Pattern insights', body: 'Guthub spots trends in your symptoms and connects them to what you ate.' },
  ...
  return (
    <section style={{
      ...
      <div aria-hidden style={{
        ...
        background: 'radial-gradient(circle, rgba(224,124,89,0.22) 0%, rgba(224,124,89,0) 70%)',
        pointerEvents: 'none',
      }} />
      <div aria-hidden style={{
        position: 'absolute', bottom: -180, left: -140, width: 460, height: 460,
        background: 'radial-gradient(circle, rgba(244,208,162,0.16) 0%, rgba(244,208,162,0) 70%)',
        pointerEvents: 'none',
      }} />
      ...
      <div style={{ maxWidth: 'var(--maxw-page)', margin: '0 auto', position: 'relative' }}>
        <Reveal style={{ textAlign: 'center', marginBottom: 64, maxWidth: 640, marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <h2 style={{
            fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem, 3.4vw, 2.75rem)',
            lineHeight: 1.18, letterSpacing: '-0.02em', fontWeight: 400,
            marginTop: 20, color: 'var(--cream-100)',
            textWrap: 'balance',
          }}>
...
function IncludedCard({ f, featured }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: featured
          ? 'var(--terracotta-400)'
          : (hover ? 'rgba(250, 245, 238, 0.10)' : 'rgba(250, 245, 238, 0.06)'),
        border: featured ? 'none' : '1px solid rgba(250, 245, 238, 0.12)',
      ...
      <div style={{
        ...
        background: featured ? 'rgba(255,255,255,0.18)' : 'var(--terracotta-400)',
        display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20,
        transform: hover ? 'scale(1.05)' : 'scale(1)',
        transition: 'transform 320ms var(--ease-out)',
      }}>
        <i data-lucide={f.icon} style={{ width: 24, height: 24, color: '#fff' }}></i>
      </div>
      <div style={{
      ...
      <div style={{
        fontSize: 15,
        color: featured ? 'rgba(255,255,255,0.88)' : 'rgba(250, 245, 238, 0.72)',
        lineHeight: 1.55,
      }}>{f.body}</div>
    </div>
  );
          ...
          {items.map((it, i) => (
            <div key={i} style={{ borderTop: i ? '1px solid var(--ink-100)' : 'none' }}>
              <button onClick={() => setOpen(open === i ? -1 : i)}
                ...
                <i data-lucide={open === i ? 'minus' : 'plus'} style={{ width: 20, height: 20, color: 'var(--ink-500)', flexShrink: 0 }}></i>
              </button>
              {open === i && (
                <div style={{ padding: '0 28px 24px', fontSize: 16, lineHeight: 1.6, color: 'var(--ink-700)' }}>
                  {it.a}
                </div>
              )}
            </div>

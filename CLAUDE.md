...
- **Hero media strip (TV stations)** — trust bullets removed from bottom of Hero. Replace with a press/media logos strip featuring TV stations Alina has appeared on. Content TBD from user.
- **Leaked password protection (Supabase)** — requires Supabase Pro plan. Enable once upgraded: Authentication → Attack Protection → toggle on "Leaked Password Protection". This checks new passwords against the HaveIBeenPwned breach database at signup/password change. No code changes needed — dashboard toggle only.
...
- **App guide page (marketing site)** — a "Getting started" or "How to get the most out of Guthub" page with visual workflow walkthrough. Defer until 20-30 paying users exist and real confusion patterns emerge from feedback. Will double as SEO content.
